...
\item
  ...
  Spatial interaction models (SIMs) definition - first intuitive definition / explanation - introduce the idea that it captures interaction between places e.g.~human, information, knowledge and trade flows -conceptually describe what spatial interaction modelling is (including its key components: distance and gravity forces).
  Describe what is needed in practice - origin-destination data.
\item
  Why are SIMs important?
  Applications and context - what they allow us to do: retail (delineating market areas), migration, human mobility, transport (factors influencing the size of people travelling), urban planning (), what if scenarios (forecasting) - forecasting/prediction applications to plan for new services, critical distance thresholds to delineate geographical areas of service,
\item
