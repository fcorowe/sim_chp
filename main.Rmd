...
The radiation [@simini_universal_2012], exploration and preferential return [@pappalardo2015] and random utility models [@mcfadden1974analysis] have become prominent frameworks to model spatial flows, particularly movement of people.
...
<!--# Overview of new structure. Below is how I see the structure for the chapter i.e. identifying reproducibility, calibration and big data modelling as key challenges, and then a section identifying and discussing different areas of work that we consider important to address the identified challenges -->
...
-   *Big Data* modelling
    ...
    -   Challenges: lots of data but issues of scalability, inference based on P values, capturing heterogeneity and temporal trends.

    -   What is needed: different approaches - use of machine learning (because it is not possible), generalised linear mixed models (GLMMs) to capture heterogeneity and spatial non-stationary, greater use of causal inference approaches and warnings of the misuse of statistical significance and *P* values.
...
Such situation is unfortunate as it is likely to have limited the applicability and portability of sophisticated frameworks to estimate SIMs and capture the effects of spatial structure on spatial flows through the use of spatial choice, spatial econometric or eigenvector spatial filtering modelling techniques.
Even the application of basic formulations of SIMs tends to represent a daunting challenge for beginning modelers of spatial flow data.
...
We argue for reproducibility because it can generate all these benefits!

Yet, reproducibility of SIM estimates is key

-   Why reproducibility is seen as a challenge / reproducing research / model estimates and why is important - not starting from scratch- build on existing work and address important issues of xxx.
    Benefits [@brunsdon2016] [@rowe2020]:

    -   enhance comparability

    -   enhance transparency / credibility - accountability, validation and verification

    -   portability - not need to rebuild; expanding existing work to address novel question and results can be updated

    -   can augment impact of research

-   What has prevented reproducibility
...
## Calibration <!--# 450 words -->

Define calibration

Defining the challenge:

-   lots of models but they are not readily accessible - focus on maths but not practice

-   Detachment between theory and practice

-   The way to calibration has been done and has not evolved much, particularly to capture patterns of heterogeneity.

-   Lack of geographical education / training

-   

## Big Data modelling <!--# 450 words -->

Rise of data imposes key challenges for the modelling of spatial interactions

-   [@oshan2021spatial] "Although local models and spatial non-stationarity were originally used to diagnose problems arising from spatial structure, there has been limited recent research using local SI models".
...
software, practical reproducible examples, open science, digital technology (including docker, computational notebooks), enhance training
...
## Capturing spatial and temporal heterogeneity <!--# 450 words -->

generalised linear mixed models (GLMMs) to capture heterogeneity and spatial non-stationary

## Enhancing statistical inference <!--# 450 words -->

greater use of causal inference approaches and warnings of the misuse of statistical significance and *P* values (We are not calling for a ban on *P* values. Nor are we saying they cannot be used as a decision criterion in certain specialized applications (such as determining whether a manufacturing process meets some quality-control standard).
...
## Integrating data science <!--# 450 words -->
...
<!--# I plan to use the text below to fill the sections above -->
