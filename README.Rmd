...
The *prediction* of the size and direction of spatial flows has been used to predict the impact of new service units, such as shopping stores, healthcare facilities and housing units on the potential demand for associated services and traffic patterns.
...
Third, measures of spatial separation have also been refined to more appropriately reflect the geographical, physiological and financial distance and costs between origins and destinations [@schwartz1973interpreting]. It is now much easier to calculate route distances on road networks thanks to routing engines and interfaces to routing engines from languages in which SIMs can be developed [@pereira2021, @morgan_opentripplanner_2019].
Many innovations are possible thanks to new datasets, for example the use of night-time light intensity to estimate the spatial distribution of human settlements [@niedomysl2017], and the use of aggregated datasets derived from social media sources to train SIMs [@lovelace_geotagged_2014].
...
. `simodels` enables the rapid develop of SIMs --- starting from basic geographic datasets as the key input --- in comparatively few lines of code
.
...
The first step to using the package and reproducing the minimal results shown below is to install R and an appropriate integrated development environment such as RStudio.
After starting RStudio for the first time, you can get started with SIMs in a statistical programming environment that has strong support geographic visualisation (as we will see in the visualisations in this section) and modelling capabilities.
The first step in this reproducible example is to install the package by entering and running the following command into the R console:
...
While is not a major reduction, the process can greatly speed-up SIM processing, modelling and visualisation run times dealing with large data sets, tackling issues outlined in Section \@ref(big-data-modelling).
...
In terms of the `simodels` package briefly introduced in this section, it is early days (the package was first published on CRAN in June 2022).
